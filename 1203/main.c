	...
	while(1)
		{
			...
			{
				fr = open_append(&fil, "testDriveGPS.txt");
				if (fr == FR_OK)
				{
					f_printf(&fil,"%d %f %f\n", node++, GPS_data.latitudeDegrees, GPS_data.longitudeDegrees);
					f_close(&fil);
				}
			f_mount(0, "", 1);
			}
